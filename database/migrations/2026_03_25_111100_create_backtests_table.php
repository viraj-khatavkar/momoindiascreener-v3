...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('backtests', function (Blueprint $table) {
            ...
            $table->unsignedInteger('max_stocks_to_hold');
            $table->unsignedInteger('worst_rank_held');
            $table->string('rebalance_frequency');
            $table->unsignedTinyInteger('rebalance_day');
            $table->string('weightage');
            $table->string('cash_call');
            $table->string('cash_call_index')->default('nifty-50');
            $table->decimal('initial_capital', 20, 2)->default(5000000.00);
            $table->timestamp('started_at')->nullable();
            $table->timestamp('completed_at')->nullable();
            $table->text('error_message')->nullable();

            // Screen filter columns (copied from screens table)
            $table->string('index');
