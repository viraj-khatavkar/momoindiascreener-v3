<template>
    <div>
        ...
        <div class="flex items-start justify-between">
            <PageHeader description="Edit your backtest">{{ backtest.name }}</PageHeader>
        </div>
        ...
        <form @submit.prevent="update">
            ...
            <div class="mt-4 flex gap-4">
                ...
                <button
                    ...
                    class="cursor-pointer rounded-md bg-green-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-green-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-600 disabled:cursor-not-allowed disabled:opacity-75"
                    @click="runBacktest"
                >
                    Run Backtest
                </button>
...
const matchingOption = medianVolumeOneYearOptions.filter(
    (option) => option.id == props.backtest.median_volume_one_year,
);

const selectedMedianVolumeOption = ref(
    matchingOption.length === 1 ? props.backtest.median_volume_one_year : 'custom',
);
...
function update(): void {
    form.put(`/backtests/${props.backtest.id}`, {
        preserveScroll: true,
    });
}
