<template>
    <div>
        <Head title="Edit Corporate Action" />
        <PageHeader :description="`Edit the corporate action for ${corporateAction.symbol}.`">
...
const props = defineProps<{
    corporateAction: BacktestNseCorporateAction;
    types: string[];
}>();
...
const form = useForm({
    date: props.corporateAction.date.slice(0, 10),
    symbol: props.corporateAction.symbol,
    series: props.corporateAction.series ?? '',
    type: props.corporateAction.type ?? '',
    description: props.corporateAction.description ?? '',
    ratio: props.corporateAction.ratio ?? '',
    dividend: props.corporateAction.dividend ?? '',
    dividend_adjustment_factor: props.corporateAction.dividend_adjustment_factor ?? '',
    price_adjustment_factor: props.corporateAction.price_adjustment_factor ?? '',
});

function update() {
    form.put(`/admin/corporate-actions/${props.corporateAction.id}`);
}
</script>
